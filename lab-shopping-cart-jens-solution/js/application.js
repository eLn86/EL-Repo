$(function() {
    ...
    /* Check Create fields Name and Price while typing */
    function checkNamePrice(){
      var name = $("input[name='name']").val();
      var price = $("input[name='price']").val();

      //Check input
      if(name.length === 0 || ($.isNumeric(name)) === true) {
        $('.create .name .form-group').addClass('has-error');
        return;
      }

      if(name.length > 0 || ($.isNumeric(name)) === false) {
        $('.create .name .form-group').removeClass('has-error');
        $('.create .name .form-group').addClass('has-success');
        return;
      }
      ...
      if(price.length > 0) {
        $('.create .price .form-group').removeClass('has-error');
        $('.create .price .form-group').addClass('has-success');
        return;
      }
    ...
    $('.create').on('keyup', '.form-control', function(event){
      checkNamePrice();
    });
